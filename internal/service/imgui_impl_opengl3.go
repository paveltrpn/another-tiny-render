...
import (
	...
	// "demos_go/internal/renderers/gl/v3.2-core/gl"
	"github.com/go-gl/gl/v4.1-core/gl"

	"github.com/inkyblackness/imgui-go/v4"
)

var unversionedVertexShader string = string(`
...
var unversionedFragmentShader string = string(`
...
func NewOpenGL3(io imgui.IO) (IImGUIRender, error) {
	// err := gl.Init()
	// if err != nil {
	...
	for _, list := range drawData.CommandLists() {
		...
		for _, cmd := range list.Commands() {
			if cmd.HasUserCallback() {
				...
				// В примере из https://github.com/inkyblackness/imgui-go-examples тут вызывается
				// функция gl.DrawElementsBaseVertexWithOffst() с 4-м аргументом типа uintptr,
				// я переделал на вызов gl.DrawElementsBaseVertex() и каст к unsafe.Pointer чтобы избавиться
				// от локальной зависимости биндингов OpenGL 3.2 из того репозитория и использовать общий
				// на всё приложение пакет биндингов OpenGL.
				gl.DrawElementsBaseVertex(gl.TRIANGLES, int32(cmd.ElementCount()), uint32(drawType),
					gl.Ptr(uintptr(cmd.IndexOffset()*indexSize)), int32(cmd.VertexOffset()))
			}
		}
	}
